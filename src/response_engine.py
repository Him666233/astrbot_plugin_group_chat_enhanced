...
class ResponseEngine:
    ...
    def __init__(self, context: Context, config: Any, image_processor=None):
        """
        初始化回复引擎。
        ...
        Args:
            ...
            image_processor: 图片处理器实例，用于处理消息中的图片。
        """
    ...
    def _summarize_chat_context(self, chat_context: Dict[str, Any]) -> str:
        """摘要化聊天上下文信息，用于详细日志输出。"""
        try:
            summary = {
                "user_impression": chat_context.get("user_impression", {}),
                "conversation_history_count": len(chat_context.get("conversation_history", [])),
                "relevant_memories_count": len(chat_context.get("relevant_memories", [])),
                "group_activity": chat_context.get("group_activity", 0.0),
                "fatigue_level": chat_context.get("fatigue_level", 0.0),
                "interaction_mode": chat_context.get("interaction_mode", "normal")
            }
            return str(summary)
        except Exception:
            return "无法摘要聊天上下文"
    ...
    async def generate_response(self, event: Any, chat_context: Dict[str, Any], willingness_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self._is_detailed_logging():
            logger.debug(f"ResponseEngine: 详细日志 - 意愿计算结果: {willingness_result}")
            logger.debug(f"ResponseEngine: 详细日志 - 聊天上下文摘要: {self._summarize_chat_context(chat_context)}")
    ...
    async def _generate_with_air_reading(self, event: Any, chat_context: Dict[str, Any], willingness_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 检查是否启用详细日志
        if self._is_detailed_logging():
            logger.debug(f"ResponseEngine: 详细日志 - 读空气决策开始，事件: {event.get_sender_id()}, 群组: {event.get_group_id()}")
...
请开始你的判断和回复："""
    ...
    async def _call_llm_for_air_reading(self, prompt: str, event: Any) -> str:
        ...
        try:
            ...
            # 检查是否启用详细日志
            if self._is_detailed_logging():
                logger.debug(f"ResponseEngine: 详细日志 - 读空气提示词长度: {len(prompt)}")
                logger.debug(f"ResponseEngine: 详细日志 - 图片URL数量: {len(image_urls)}")
                logger.debug(f"ResponseEngine: 详细日志 - 提供商类型: {type(provider).__name__}")
            ...
            # 检查是否启用详细日志
            if self._is_detailed_logging():
                logger.debug(f"ResponseEngine: 详细日志 - 系统提示词长度: {len(sys_prompt)}")
                logger.debug(f"ResponseEngine: 详细日志 - 人格信息: {persona}")
            ...
            # 检查是否启用详细日志
            if self._is_detailed_logging():
                logger.debug(f"ResponseEngine: 详细日志 - 异常详细信息: {str(e)}")
